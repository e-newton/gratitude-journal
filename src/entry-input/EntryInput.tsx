...
export default function EntryInput(props: EntryInputProps) {
    ...
    return(
        <CSSTransition nodeRef={nodeRef} in={props.selected} classNames="selected" addEndListener={() => undefined} appear={true}>
            <textarea className={className()} autoFocus={props.selected} ref={nodeRef} rows={4} onChange={onInputChange} onFocus={() => props.onFocus(props.id)}></textarea>
        </CSSTransition>
    );
}